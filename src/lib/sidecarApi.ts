...
export const sidecarApi = {
  seedData: (payload: { target: string; target_name: string; connection_string?: string; custom_schema?: Record<string, any> }) => requestSidecar<{ status: string; message: string }>('/api/extensions/seed-data', {
    method: 'POST',
