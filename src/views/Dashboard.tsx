...
const Dashboard = () => {
  ...
  const { s3, dynamo, lambda, sqs, codeartifact, loading: statsLoading } = useDashboardStats();
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6 bg-brand-console text-brand-green border-brand-green/20">
               ...
               <div className="flex flex-wrap gap-2 text-[9px] font-mono">
                 {['S3', 'Lambda', 'SQS', 'SNS', 'DynamoDB', 'IAM', 'SecretsMgr', 'CodeArtifact', 'EC2', 'ECS', 'STS'].map(s => (
                   <span key={s} className="px-2 py-0.5 border border-brand-green/30 rounded-full lowercase tracking-tighter">[{s}]</span>
                 ))}
                 <span className="opacity-40">+42 more</span>
               </div>
            </Card>
          </div>
        </div>
